...
titleformat_text_filter_impl_reserved_chars_ex::titleformat_text_filter_impl_reserved_chars_ex(const char * p_reserved_chars, const char * p_replacement_chars)
{
    ...
    {
        unsigned c1, c2;
        t_size len1 = utf8_decode_char(p_reserved_chars, c1);
        if (len1 == 0) break;
        t_size len2 = utf8_decode_char(p_replacement_chars, c2);
...
void titleformat_text_filter_impl_reserved_chars_ex::write(const GUID & p_inputtype, string_receiver & p_out, const char * p_data, t_size p_data_length)
{
...
void titleformat_text_filter_impl_reserved_chars_ex::replace_forbidden_chars(string_receiver & p_out, const char * p_source, t_size p_source_len) const
{
    ...
    else
    {
        ...
        {
            unsigned c1, c2;
            t_size delta = utf8_decode_char(p_source + index, c1, p_source_len - index);
            if (delta == 0) break;
